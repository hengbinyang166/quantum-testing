...
MIN_LARGE_QUBITS = 20
MAX_LARGE_QUBITS = 20 # 设置为-1或小于MIN_LARGE_QUBITS的值，代表一直运行到芯片最大比特数
...
def main():
    ...
    for q_large in range(MIN_LARGE_QUBITS, run_until_qubits + 1):
        ...
        for instance_idx in range(1, NUM_INSTANCES_PER_SIZE + 1):
            ...
            if q_large == 20 and instance_idx < 6:
                print(f"  - 根据设置，跳过实例 q{q_large}_s{instance_idx}...")
                continue# 从筛选后的合格电路池中随机选择
